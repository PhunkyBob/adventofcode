...
class Coord(NamedTuple):
    ...
    def __add__(self, direction: Direction) -> "Coord":
        return Coord(self.x + direction.value[0], self.y + direction.value[1])
...
def find_shortest_path(
    ...
    if end is None:
        end = Coord(len(matrix[0]) - 1, len(matrix) - 1)
    candidates: List[Tuple[int, Element]] = [(0, Element(start, dir, 1)) for dir in Direction]
    certains = set()
    iter = 0
    while candidates:
        iter += 1
        ...
        if new_coord.x < 0 or new_coord.x >= len(matrix[0]) or new_coord.y < 0 or new_coord.y >= len(matrix):
            continue
        new_cost = cost + matrix[new_coord.y][new_coord.x]
        if min_direction_count <= current.direction_count <= max_direction_count and new_coord == end:
            return new_cost
        for new_direction in Direction:
            if (
                new_direction.value[0] == 0 - current.direction.value[0]
                and new_direction.value[1] == 0 - current.direction.value[1]
            ):
                continue
            new_direction_count = current.direction_count + 1 if new_direction == current.direction else 1
            if new_direction_count > max_direction_count or (
                current.direction != new_direction and current.direction_count < min_direction_count
            ):
                continue
            heapq.heappush(candidates, (new_cost, Element(new_coord, new_direction, new_direction_count)))
    return -1
