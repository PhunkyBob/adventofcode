...
def test_motions_move_head():
    ...
    motions = Motions("")
    motions.move_head("R")
    assert motions.head == Position(1, 0)
    motions.move_head("U")
    assert motions.head == Position(1, 1)
    motions.move_head("L")
    assert motions.head == Position(0, 1)
    motions.move_head("D")
    assert motions.head == Position(0, 0)


def test_motions_move_tail():
    """Tests if the tail folows."""
    motions = Motions("")
    motions.head = Position(2, 1)
    motions.tail = Position(1, 1)
    motions.move_head("R")
    assert motions.tail == Position(2, 1)

    motions.head = Position(1, 2)
    motions.tail = Position(1, 3)
    motions.move_head("D")
    assert motions.tail == Position(1, 2)

    # Move diagonally
    motions.head = Position(2, 2)
    motions.tail = Position(1, 1)
    motions.move_head("U")
    assert motions.tail == Position(2, 2)
